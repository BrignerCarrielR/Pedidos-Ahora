export const queries = {
    ...
    consultarCarritos: 'select mc.nombre_plato,mc.descripcion , dc.cantidad, dc.precio, dc.total from detalle_carrito dc, menu_comidas mc, carrito c, usuarios u where u.id = c.id_usuario and c.id = dc.id_carrito and dc.id_menu_comida = mc.id and u.id = $1',
    callAgregarCarrito: 'CALL agregar_al_carrito($1, $2, $3)',

    buscarRefreshToken: 'UPDATE usuarios SET refresh_token = $1 WHERE id = $2',

    consultarPedidos: 'select p.id, u.nombreusuario, fecha_pedido, p.estado, p.total from pedidos p, usuarios u where u.id = p.id_usuario order by estado desc ',
